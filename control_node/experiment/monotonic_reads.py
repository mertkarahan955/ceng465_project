...
def run_monotonic_reads():
    ...
    # Oplog arrow: primary → secondary (single arrow representing v1→v5 stream)
    events.append({
        "t_ms":       oplog_start_ms,
        "latency_ms": safe_lat(net_s),
        "from":       "primary",
        "to":         "secondary",
        "label":      f"oplog v1→v{FINAL_VERSION} (async, ~{repl_ms:.1f}ms)",
        "type":       "replicate",
    })
    # Applied ack: secondary signals it has applied the write — closes the span band
    events.append({
        "t_ms":       secondary_applied_ms,
        "latency_ms": safe_lat(net_s),
        "from":       "secondary",
        "to":         "primary",
        "label":      f"applied — {total_repl_ms:.0f}ms after write",
        "type":       "ack",
    })

    # 5 sequential reads from SECONDARY
    prev_v = None
    for r in reads:
        v         = r["version"]
        s         = r["status"]
